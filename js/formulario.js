document.addEventListener("DOMContentLoaded", function () {
    ...
    inputs.forEach((input)document.addEventListener("DOMContentLoaded", function () {
		const formulario = document.getElementById('formulario');
		const inputs = document.querySelectorAll('#formulario input');
		...
		const validarCampo = (expresion, input, campo) => {
			...
			if (!grupoCampo) return; // Verifica si el grupo del campo existe
			if (expresion.test(input.value)) {
				...
				if (mensajeError) mensajeError.classList.remove('formulario__input-error-activo');
				campos[campo] = true;
			} else {
				...
				const mensajeError = grupoCampo.querySelector('.formulario__input-error');
				if (mensajeError) mensajeError.classList.add('formulario__input-error-activo');
				campos[campo] = false;
			}
		}
		...
		const validarPassword2 = () => {
			...
			if (!grupoPassword2) return; // Verifica si el grupo del campo existe
			if (inputPassword1.value !== inputPassword2.value) {
				...
				const mensajeError = grupoPassword2.querySelector('.formulario__input-error');
				if (mensajeError) mensajeError.classList.add('formulario__input-error-activo');
				campos['password'] = false;
			} else {
